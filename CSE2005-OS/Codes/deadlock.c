...
int ALLOC[N][N];	// Allocated matrix
int MAX[N][N];	// Max needed
int NEED[N][N];	// Need matrix
int AVAIL[N];		// Availibility matrix
int TOTAL[N];		// Total system resources
...
void printmat (int mat[N][N], int m, int n) {
	int i, j;
	for (i = 0; i < n; ++i) {
		printf("P%d |", i);
		for (j = 0; j < m; ++j) {
